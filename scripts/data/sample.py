...
def generate_people(count: int, verbose: bool = False) -> None:
    ...
    try:
        ...
        print(
            f"✅ Generated {len(people_data)} person records in {execution_time:.1f}s"
        )

        if verbose:
            print(f"   Rate: {len(people_data) / execution_time:.0f} records/sec")
            print(
                f"   Sample record keys: {list(people_data[0].keys()) if people_data else 'None'}"
            )
            ...
            if people_data and len(people_data) > 0:
                ...
                print(
                    f"     Name: {sample.get('first_name', 'N/A')} {sample.get('last_name', 'N/A')}"
                )
                print(f"     Email: {sample.get('email', 'N/A')}")
                print(
                    f"     Location: {sample.get('city', 'N/A')}, {sample.get('state', 'N/A')}"
                )
...
def generate_companies(count: int, verbose: bool = False) -> None:
    ...
    try:
        ...
        print(
            f"✅ Generated {len(company_data)} company records in {execution_time:.1f}s"
        )

        if verbose:
            print(f"   Rate: {len(company_data) / execution_time:.0f} records/sec")
            print(
                f"   Sample record keys: {list(company_data[0].keys()) if company_data else 'None'}"
            )
            ...
            if company_data and len(company_data) > 0:
                ...
                print(f"     Name: {sample.get('company_name', 'N/A')}")
                print(f"     Industry: {sample.get('industry', 'N/A')}")
                print(
                    f"     Location: {sample.get('headquarters_city', 'N/A')}, {sample.get('headquarters_state', 'N/A')}"
                )
...
def generate_full_dataset(
    ...
    try:
        ...
        if template:
            ...
            print(
                f"   Custom counts - People: {people_count}, Companies: {companies_count}"
            )
        ...
        if template:
            ...
            dataset = quick_generate_full_dataset(
                seed=42, people=people_count, companies=companies_count
            )
        ...
        for template_name in templates:
            if verbose:
                try:
                    ...
                    print(
                        f"  • {template_name}: {info.get('description', 'No description')}"
                    )
                    workflows = info.get("workflows", {})
                    workflow_summary = ", ".join(
                        [f"{k}: {v}" for k, v in workflows.items()]
                    )
...
def main():
    parser = ArgumentParser(
        description="Generate sample data using built-in quick functions"
    )
    parser.add_argument(
        "--type",
    ...
    parser.add_argument(
        "--count", type=int, help="Number of records to generate (for people/companies)"
    )
    parser.add_argument(
        "--people",
        type=int,
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Verbose output with sample data"
    )
