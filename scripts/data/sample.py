...
def generate_people(count: int, verbose: bool = False) -> None:
    ...
    try:
        ...
        config: WorkflowConfig = WorkflowConfig(batch_size=min(50, count), seed=42, write_mode='truncate')
        workflow : PeopleWorkflow= PeopleWorkflow(config, DatabaseConfig.for_dev())
        
        result: WorkflowResult = workflow.execute(count)
        
        print(f"✅ Generated {result.records_generated} person records in {result.execution_time:.1f}s")
        
        if verbose:
            print(f"   Batch size: {config.batch_size}")
            print(f"   Rate: {result.records_generated / result.execution_time:.0f} records/sec")
...
def generate_companies(count: int, verbose: bool = False) -> None:
    ...
    try:
        ...
        config: WorkflowConfig = WorkflowConfig(batch_size=min(10, count), seed=42, write_mode='truncate')
        workflow: CompaniesWorkflow = CompaniesWorkflow(config, DatabaseConfig.for_dev())
        
        result: WorkflowResult= workflow.execute(count)
        
        print(f"✅ Generated {result.records_generated} company records in {result.execution_time:.1f}s")
...
def main():
    ...
    parser.add_argument('--type', choices=['people', 'companies', 'dataset'], 
